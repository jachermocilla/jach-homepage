...
on Unix and is POSIX compliant.Since its initial release, Minix has grown
in size in terms of source code as well as in complexity which makes it
difficult to use for teaching. 
...
The Nachos kernel and hardware simulator runs as Unix processes. Success
stories in using Nachos for teaching can be found in \cite{gary:nachos}.
The fact that Nachos runs as a Unix process limits its appeal because it cannot
run on a real hardware and dependent on the host.
...
\subsection{DEX-OS}
DEX-OS\cite{dayo:dexos} was developed by Joseph Emmanuel Dayo. Dex-OS was opensourced under 
