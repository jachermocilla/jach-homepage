...
for the students to understand how a PC boots and an operating system is loaded
A simple bootloader was developed in assembly language. The exercise was then 
extended so that the CPU goes into the protected mode instead of the default 
real mode. This is because modern operating systems run in protected as with
the case of ICS-OS.
...
\subsubsection{Adding a \textit{hello} shell command}
The next activity is for the students to modify the kernel, particularly 
the shell, by adding an internal command. The students was guided to 
navigate to the console source code (kernel/console/console.c). They were
asked to modify the function  console\_execute() by adding the following code
fragment.
