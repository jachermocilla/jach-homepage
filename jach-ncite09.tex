...
its appeal because it cannot run on a real hardware and dependent on the host.
...
\subsection{DEX-OS}
DEX-OS\cite{dayo:dexos} is an educational operating system based on
...
modern operating systems like Linux and Windows. ICS-OS is mainly based
on DEX-OS. The following are the main components of DEX-OS.
...
A convenient environment for operating system development is essential. The 
compilers and other build tools should be readily available at the development
machine. Thus, a linux environment was chosen.

The original development environment for DEX-OS was in Windows and uses the 
Dev-C++ IDE. Since the laboratory uses linux, the first step was to migrate
the development environment to linux.
