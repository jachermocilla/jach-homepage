...
After extracting, the students built the kernel image using the \textit{make}
command. The sequence of commands below builds and tests ICS-OS
\begin{verbatim}
(1) $make
(2) $sudo make install
(3) $bochs -q
...
The second command builds the distribution floppy image and thus requires 
adminstrator privileges so the \textit{sudo} command is used. The third command
starts the Bochs\cite{bochs:site} which loads ICS-OS.

\subsection{Adding a new internal shell command}
