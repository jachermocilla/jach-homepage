...
The first command creates the kernel binary.The second command builds the
distribution floppy image using the loopback device for mounting and thus 
requires adminstrator privileges so the \textit{sudo} command is used. The
third command starts Bochs\cite{bochs:site} which loads ICS-OS.
