...
to write and modify kernel code. The entire distribution fits in a 
floppy disk.

ICS-OS is divided into two main components, the kernel and user applications.
The kernel is loaded at bootup using GRUB as the bootloader. Essentially, the
kernel stays in the main memory of the computer until shutdown. Since GRUB 
supports compressed kernel images, it further reduces the size of the kernel.
